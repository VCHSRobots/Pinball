...
char lamps[] = {PIN_LAMP_1, PIN_LAMP_2, PIN_LAMP_3};
...
void set_coil(int icoil, int strength) {
  int pin = coils[icoil];
  int lamp_pin = lamps[icoil];
  if (strength <= 0) {digitalWrite(pin, LOW); digitalWrite(lamp_pin, LOW); return; }
  if (strength >= 255) {digitalWrite(pin, HIGH); digitalWrite(lamp_pin, HIGH); return; }
